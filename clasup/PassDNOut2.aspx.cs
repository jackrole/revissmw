...
{
  public partial class PassDNOut2 : System.Web.UI.Page
  {
    protected string connOLE = g.getConfig("dbQYSJ");
    protected void Page_Load(object sender, EventArgs e)
    {
      if (!Page.IsPostBack) LoadDN();
    }
    ...
    private void LoadDN()
    {
      ...
      string strsql = g.getSValue(string.Format("exec wms2_p_CHK1 '{0}'", g.ToSql(passID)));
      ...
      string xmlPath = Server.MapPath("../xml/") + passID + "/out";
      if (!Directory.Exists(xmlPath) || Directory.GetFiles(xmlPath).Length == 0)
      {
        if (!Directory.Exists(xmlPath))
          Directory.CreateDirectory(xmlPath);
        CreateXML(passID);
      }
      string fileName = Server.MapPath("../xml/") + passID + "/out.zip";
      if (!File.Exists(fileName))
        ClsZip.PackFiles(fileName, xmlPath);
      g.Download(fileName, passID + "_out.zip");
    }

    private void CreateXML(string passID)
    {
      StringBuilder sb;

      int k = 0, m = 0, iRow = 0, pcs = 0;
      string strsql;
      decimal nw = 0, gw = 0;
      ...
      string DNO = "", DELIVERY_NO = "", bgdno = "", wbook, xmlPath, dbPath, filePath = Server.MapPath("../xml/") + passID + "/out";
...
'{10}', '{11}', '{12}', '{13}', '{14}', '{15}', '{16}', {17}, '', '', '', '')";
      ...
      for (int i = 0; i < dtt.Rows.Count; i++)
      {
        ...
        g.CreateFile(string.Format("{0}/{1}", filePath, xmlPath), sb);

        wb.Save(string.Format("{0}/调拨单{1}.xls", filePath, i), SaveFormat.Excel97To2003);
      }

      File.Copy(dbPath, string.Format("{0}/QYSJ.mdb", filePath));
      File.Copy(dbPath1, string.Format("{0}/区内调拨.mdb", filePath));
    }
